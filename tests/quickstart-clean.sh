...
SOURCE_DATE_EPOCH=0 "$project_root/scripts/package-release.sh" v0.1.2 "$test_root/dist" >/dev/null
(
  cd "$test_root/dist"
  sha256sum -c SHA256SUMS >/dev/null
)
tar -xzf "$test_root/dist/github-sarif-preflight_v0.1.2_linux_arm64.tar.gz" -C "$test_root"
package_root="$test_root/github-sarif-preflight_v0.1.2_linux_arm64"
